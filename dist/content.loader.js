 ...
 */
export const findPageContent = async (searchPath) => {
 ...
 */
export const parseFragment = async (obj) => {
    if (!obj ||
        typeof obj !== 'object' ||
        Array.isArray(obj) ||
...
export const validateAndTransformComponent = async (content) => {
    const path = `${config.componentRoot}/${content.component}.svelte`;
    let schema = await getSchema(path);
    if (!schema?.spa) {
        return content;
    }
    const result = await schema.spa(content);
    if (!result.success) {
        throw new Error(`Component '${content.component}' failed validation: ${result.error.message}`);
    }
    return result.data;
};
const processVirtualComponent = async (content) => {
