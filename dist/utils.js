 ...
 */
export const contentTraverser = async ({ obj, callback, filter, deep = true }) => {
    if (Array.isArray(obj)) {
        const newArr = await Promise.all(obj.map((item) => contentTraverser({ obj: item, filter, callback, deep })));
        return newArr;
    }
