...
async function plugin(config) {
    ...
    const getContent = (refresh = false) => {
        if (refresh || !content) {
            ...
            content = Object.fromEntries(currentEntries.map((path) => {
                const pagePromise = loadContent(path, config, 
                ...
                (processedVirtualComponent) => {
                    ...
                    if (processedVirtualComponent?.component) {
                        ...
                        console.warn(`Processed virtual component from page ${path} lacks a component name.`, processedVirtualComponent);
                    }
                });
    ...
    return {
        ...
        async handleHotUpdate({ file, modules, server }) {
            ...
            if (contentFiles.has('composably:content')) {
                modulesToRefresh.push(...server.moduleGraph.getModuleById('composably:content'));
            }
