...
export const parse = async (content) => {
    try {
        ...
        content.html = content.html.replace(/<svelte-component data-slot="([^"]+)"([^>]*)><\/svelte-component>/g, '<slots.$1.component {...slots.$1} />');
        ...
        throw new Error(`Failed to parse input: "${content.markdown.slice(0, 50)}" - ${error}`);
    }
};
